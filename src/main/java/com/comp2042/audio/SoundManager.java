...
public final class SoundManager {
    ...
    private static AudioClip load(String fileName) {
        try {
            ...
            if (resource == null) {
                System.err.println("SoundManager: Sound file not found: /sound/" + fileName);
                return null;
            }
            System.out.println("SoundManager: Loading sound file: " + fileName + " from " + resource);
            AudioClip clip = new AudioClip(resource.toExternalForm());
            clip.setVolume(SFX_VOLUME);
            System.out.println("SoundManager: Successfully loaded: " + fileName);
            return clip;
        } catch (Exception e) {
            System.err.println("SoundManager: Failed to load sound " + fileName + ": " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
    ...
    public static void playRotate() {
        play(MOVE_CLIP); // Use same sound as move
    }
